...
def dataset_split(X, y, ratio=0.8):
    ...
    print
    "Records in Training Dataset", X_train.shape[0]
    print
    "Records in Validation Dataset", X_val.shape[0]
...
class DNN(object):
    ...
    def predict(self, X):
        ...
        '''
        labels = unpickle("./Assignment One/cifar-10-python/cifar-10-batches-py/batches.meta")["label_names"]
        preds = np.array([])
        for x in X:
            outs, activations = self.feedforward(x)
